...
Market Depth DELAY
    ...
    Log
    ...    Expected: trade_price=${expect_trade_price}, change_point=${expect_change_point}, change_percent=${expect_change_percent}
    Log
    ...    Actual: trade_price=${actual_lastTradePrice}, change_point=${actual_changePoint}, change_percent=${actual_changePercent}
    Should Be Equal    ${actual_lastTradePrice}    ${expect_trade_price}    msg=Trade price mismatch
    Should Be Equal    ${actual_changePoint}    ${expect_change_point}    msg=Change point mismatch
    Should Be Equal    ${actual_changePercent}    ${expect_change_percent}    msg=Change percent mismatch
...
Market Depth 0026 0027 0028 0029 0030 0031 0032 0033 0034 - CLICK TO REFRESH
    ...
    ${token}=    Get token web trading    ${apiUrl}    ${username}    ${password}    ${origin}    ${version}    ${env}
